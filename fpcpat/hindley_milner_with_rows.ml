...
let rec is_val context exp =
  match Exp.out exp with
  | Var var -> Map.mem context var
  | Fun _ -> true
  | Record fields ->
    List.for_all fields ~f:(fun (_, exp) ->
      is_val context exp)
  | Inj (_, exp) -> is_val context exp
  | Fix (var, exp) ->
    (* For the purposes of the value restriction, it's safe to consider [G |- fix x is v val] as
       long as [G |- v val], where importantly, we _don't_ add [x val] to the context. The
       justifcation is that in one step of evaluation we have [fix x is v |--> [fix x is v/x]v],
       and [G |- [fix x is v/x]v val] when [G |- v val]. *)
    ignore var;
    is_val context exp
  | Fold exp -> is_val context exp
  | Ascribe (exp, _) -> is_val context exp
  | Ap _ | Match _ | Let _ | Let_type _ -> false
;;
...
let rec infer_for_exp context exp =
  ...
  | Let ((pat, exp1), exp2) ->
    ...
    let pat_context =
      (* CR wduff: I think it's fine to pretend that the results of a finite unpacking of a value
         are values? *)
      match is_val context exp1 with
      | true -> Map.map pat_context ~f:(generalize context)
      | false -> Map.map pat_context ~f:don't_generalize
    in
    let context =
      Map.merge_skewed context pat_context ~combine:(fun ~key:_ _ _ -> assert false)
    in
